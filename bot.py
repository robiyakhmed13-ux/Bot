...
@dataclass
class Category:
    id: str; name_uz: str; name_ru: str; name_en: str; emoji: str; keywords: List[str]; tx_type: TxType

@dataclass
class PendingTx:
    category_id: str; tx_type: TxType

@dataclass  
class PendingDebt:
    debt_type: str; person_name: Optional[str] = None
...
async def ensure_user(uid,name):
    ...
    except: return {"telegram_id":uid,"balance":0,"language":"uz"}
...
async def save_tx(uid,desc,amt,cid,src="text"):
    try:
        supabase.table("transactions").insert({"user_telegram_id":uid,"description":desc,"amount":amt,"category_id":cid,"source":src}).execute()
        try: supabase.rpc("update_balance",{"p_telegram_id":uid,"p_amount":amt}).execute()
        except:
            bal=await get_bal(uid)
            supabase.table("users").update({"balance":bal+amt}).eq("telegram_id",uid).execute()
        return True
    except Exception as e: logger.error(f"save_tx:{e}"); return False
...
async def today_stats(uid):
    try:
        ...
        if r.data:
            ...
            return {"e":abs(int(row.get("total_expenses",0))),"i":int(row.get("total_income",0)),"c":int(row.get("transaction_count",0))}
    except: pass
    return {"e":0,"i":0,"c":0}
...
async def period_stats(uid,days):
    try:
        ...
        e,i,bc=0,0,{}
        for tx in (r.data or []):
            a=int(tx.get("amount",0)); c=tx.get("category_id","other")
            if a<0: e+=abs(a); bc[c]=bc.get(c,0)+abs(a)
            else: i+=a
        return {"e":e,"i":i,"c":len(r.data or []),"bc":bc}
    except: return {"e":0,"i":0,"c":0,"bc":{}}
...
async def save_debt(uid,person,amt,dtype):
    try: supabase.table("debts").insert({"user_telegram_id":uid,"person_name":person,"amount":amt,"type":dtype}).execute(); return True
    except: return False

async def update_lang(uid,lang):
    try: supabase.table("users").update({"language":lang}).eq("telegram_id",uid).execute()
    except: pass
...
async def cb(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if d=="m:exp": await q.edit_message_text(t(l,"sel_exp"),reply_markup=cat_kb(TxType.EXPENSE,l)); return
    if d=="m:inc": await q.edit_message_text(t(l,"sel_inc"),reply_markup=cat_kb(TxType.INCOME,l)); return
    if d=="m:goals":
        goals=await get_goals(usr.id)
        if not goals: await q.edit_message_text(t(l,"no_goals"),reply_markup=main_kb(l))
        else:
            txt="🎯 *Goals*\n"
            for g in goals[:5]:
    ...
    if d=="m:debts":
        ...
        if not debts: await q.edit_message_text(t(l,"no_debts"),reply_markup=main_kb(l))
        else:
            txt="📋 *Debts*\n"
            for db in debts[:5]: txt+=f"\n{'🤝' if db.get('type')=='borrowed' else '💸'} {db.get('person_name','')}: {fmt(db.get('amount',0))}"
            await q.edit_message_text(txt,parse_mode="Markdown",reply_markup=main_kb(l))
        return
    if d=="m:rep": await q.edit_message_text(t(l,"reports"),reply_markup=rep_kb(l)); return
    if d=="m:set": await q.edit_message_text("🌐",reply_markup=set_kb(l)); return
    if d.startswith("c:"):
        _,ty,cid=d.split(":")
        user_pending[usr.id]=PendingTx(cid,TxType(ty))
        c=CATEGORIES.get(cid)
        if c: await q.edit_message_text(t(l,"enter_amt").format(e=c.emoji,n=cn(c,l)))
        return
    if d.startswith("l:"):
        nl=d.split(":")[1]
    ...
    if d=="export":
        ...
        if csv_data:
            f=io.BytesIO(csv_data.encode('utf-8')); f.name=f"hamyon_{datetime.now().strftime('%Y%m%d')}.csv"
...
async def text_handler(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    usr=u.effective_user; msg=u.message
    ...
    if usr.id in user_pending:
        ...
        amt=parse_amt(txt)
        if not amt: await msg.reply_text(t(l,"cant")); return
        final=-abs(amt) if pend.tx_type==TxType.EXPENSE else abs(amt)
        c=CATEGORIES.get(pend.category_id)
        if not c: return
        await save_tx(usr.id,cn(c,l),final,pend.category_id,"manual")
    ...
    amt=parse_amt(txt)
    if not amt: await msg.reply_text(t(l,"cant"),reply_markup=main_kb(l)); return
    cid,ty=detect(txt)
    final=-abs(amt) if ty==TxType.EXPENSE else abs(amt)
    c=CATEGORIES.get(cid)
    await save_tx(usr.id,txt,final,cid,"text")
    bal=await get_bal(usr.id)
...
async def voice_handler(u:Update,ctx:ContextTypes.DEFAULT_TYPE):
    ...
    l=await get_lang(usr.id)
    if not openai_client: await msg.reply_text("⚠️ Voice not available"); return
    ...
    try:
        ...
        txt=resp.text
        amt=parse_amt(txt)
        if not amt: await msg.reply_text(f"🎤 _{txt}_\n\n{t(l,'cant')}",parse_mode="Markdown"); return
        cid,ty=detect(txt)
        final=-abs(amt) if ty==TxType.EXPENSE else abs(amt)
        c=CATEGORIES.get(cid)
        await save_tx(usr.id,txt,final,cid,"voice")
        bal=await get_bal(usr.id)
        await msg.reply_text(f"🎤 _{txt}_\n\n"+t(l,"saved").format(e=c.emoji if c else "📦",n=cn(c,l) if c else cid,i="💸" if final<0 else "💰",a=fmt(abs(final)),b=fmt(bal)),parse_mode="Markdown",reply_markup=main_kb(l))
    except Exception as e: logger.error(f"voice:{e}"); await msg.reply_text("❌ Error")
